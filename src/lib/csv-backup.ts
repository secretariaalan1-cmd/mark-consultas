...
export function exportBackupFromData(patients: Patient[], appointments: Appointment[]): string {
  let csv = '### PACIENTES ###\n';
  ...
  csv += '### AGENDAMENTOS ###\n';
  csv += ['slot', 'date', 'patientId', 'patientName', 'susCard', 'dob', 'psf', 'reason', 'type', 'time'].map(escapeCSV).join(CSV_SEP) + '\n';
  for (const a of appointments) {
    csv += [String(a.slot), a.date, a.patientId, a.patientName, a.susCard, a.dob, a.psf, a.reason, a.type, a.time || ''].map(escapeCSV).join(CSV_SEP) + '\n';
  }

  return csv;
}

export function downloadBackupFromData(patients: Patient[], appointments: Appointment[]) {
  const csv = exportBackupFromData(patients, appointments);
...
export function parseBackupCSV(text: string): { patients: Patient[]; appointments: Appointment[] } {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith('### DIAS')) { section = 'days'; continue; }
    if (section === 'days') continue;
    ...
    if (section === 'appointments' && cols.length >= 9) {
      appointments.push({
        ...
        susCard: cols[4], dob: cols[5], psf: cols[6], reason: cols[7],
        type: cols[8] as 'NORMAL' | 'RETORNO',
        time: cols[9] || '',
      });
    }
  }
